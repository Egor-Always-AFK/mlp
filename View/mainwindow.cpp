...
MainWindow::MainWindow(QWidget *parent)
  ...
  //    dir_path = "/Users/pilafber/Desktop/Projects/CPP7_MLP-0/";  // для билда
  //    в qt
  training_dataset_path_ = dir_path + "datasets/emnist-letters-train.csv";
  testing_dataset_path_ = dir_path + "datasets/emnist-letters-test.csv";
  graph_weights_path_ = dir_path + "datasets/graph_weights.txt";
  matrix_weights_path_ = dir_path + "datasets/matrix_weights.txt";
}
...
void MainWindow::on_train_network_clicked() {
  ...
  controller->Train(ui->epochs_count->value(), training_dataset_path_,
                    ui->cross_validation->isChecked(),
                    ui->number_of_groups_k->value(),
                    ui->test_group_number->value());
...
void MainWindow::UpdateTestingResultsAfterTraining() {
  ui->time_spent_on_training->setText(
      QString::number(controller->GetTimeSpentOnTraining()));
  if (ui->cross_validation->isChecked()) {
    ui->time_spent->setText(
        QString::number(controller->GetTimeSpentOnTesting()));
    ui->average_mse->setText(
        QString::number(controller->GetAverageMse(), 'f', 2));
    ui->average_accuracy->setText(
        QString::number(controller->GetAverageAccuracy(), 'f', 2));
    if (ui->precision_line_edit->text().isEmpty())
      ui->precision_line_edit->setText("a");
    ui->precision_for_letter->setText(QString::number(
        controller
            ->GetPrecisions()[QStringToInt(ui->precision_line_edit->text())],
        'f', 2));
    if (ui->recall_line_edit->text().isEmpty())
      ui->recall_line_edit->setText("a");
    ui->recall_for_letter->setText(QString::number(
        controller->GetRecalls()[QStringToInt(ui->recall_line_edit->text())],
        'f', 2));
    if (ui->f_measure_line_edit->text().isEmpty())
      ui->f_measure_line_edit->setText("a");
    ui->f_measure_for_letter->setText(QString::number(
        controller
            ->GetFMeasures()[QStringToInt(ui->f_measure_line_edit->text())],
        'f', 2));
  }
}
...
void MainWindow::UpdateTestingResultsAfterTesting() {
  ...
  if (ui->recall_line_edit->text().isEmpty())
    ui->recall_line_edit->setText("a");
  ui->recall_for_letter->setText(QString::number(
      controller->GetRecalls()[QStringToInt(ui->recall_line_edit->text())], 'f',
      2));
  if (ui->f_measure_line_edit->text().isEmpty())
    ui->f_measure_line_edit->setText("a");
  ui->f_measure_for_letter->setText(QString::number(
      controller->GetFMeasures()[QStringToInt(ui->f_measure_line_edit->text())],
      'f', 2));
}
...
void MainWindow::on_precision_line_edit_textChanged() {
  if (test_finished) {
    if (ui->precision_line_edit->text().contains(
            QRegularExpression("[А-Яа-я]"))) {
      ui->precision_line_edit->clear();
    } else if (controller->GetIsTrained() &&
               !ui->precision_line_edit->text().isEmpty()) {
      ui->precision_for_letter->setText(QString::number(
          controller
              ->GetPrecisions()[QStringToInt(ui->precision_line_edit->text())],
          'f', 2));
  ...
  if (test_finished) {
    if (ui->recall_line_edit->text().contains(QRegularExpression("[А-Яа-я]"))) {
      ui->recall_line_edit->clear();
               ...
               !ui->recall_line_edit->text().isEmpty()) {
      ui->recall_for_letter->setText(QString::number(
          controller->GetRecalls()[QStringToInt(ui->recall_line_edit->text())],
          'f', 2));
    }
...
void MainWindow::on_f_measure_line_edit_textChanged() {
  if (test_finished) {
    if (ui->f_measure_line_edit->text().contains(
            QRegularExpression("[А-Яа-я]"))) {
      ui->f_measure_line_edit->clear();
    } else if (controller->GetIsTrained() &&
               !ui->f_measure_line_edit->text().isEmpty()) {
      ui->f_measure_for_letter->setText(QString::number(
          controller
              ->GetFMeasures()[QStringToInt(ui->f_measure_line_edit->text())],
          'f', 2));
...
void MainWindow::on_restart_training_clicked() {
  ChangeAvailabilityOfButtonsAfterRestart();
}
...
void MainWindow::on_matrix_model_clicked() {
  ui->hidden_layers_count->setMaximum(1);
}

void MainWindow::on_graph_model_clicked() {
  ui->hidden_layers_count->setMaximum(5);
}
