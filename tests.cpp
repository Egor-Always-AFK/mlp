...
TEST(Test, graph) {
    ...
    SUCCEED();
    c.Train(1, "../datasets/emnist-letters-train.csv", false, 0, 0);
    c.Train(1, "../datasets/emnist-letters-train.csv", false, 0, 0);
    EXPECT_TRUE(c.GetMseForTraining().size() > 88000);
    EXPECT_TRUE(c.GetTimeSpentOnTraining() > 1);
    EXPECT_TRUE(c.SaveWeights("../datasets/test.txt"));
    // c.InitNetwork({0}, 0, true);
    EXPECT_TRUE(c.LoadWeights("../datasets/test.txt"));
    c.Test(1, "../datasets/emnist-letters-test.csv");
