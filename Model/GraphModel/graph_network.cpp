...
namespace s21 {
void GraphNetwork::InitNetwork(const std::vector<int>& config,
...
void GraphNetwork::ClearVectors(bool precisions, bool recalls, bool f_measures,
                                bool mse_for_testing, bool mse_for_training) {
...
std::vector<double> GraphNetwork::ForwardProp(
    std::vector<std::vector<double>>& letters) {
...
double GraphNetwork::Activate(std::vector<double> weights,
...
double GraphNetwork::Transfer(double activation) {
  return 1.0 / (1.0 + exp(-activation));
}

double GraphNetwork::TransferDerivative(double output) {
  ...
  for (int i = layers_.size() - 1; i >= 0; --i) {
    ...
    if (i != (int)(layers_.size() - 1)) {
      ...
      for (int j = 0; j < (int)(layer.neurons.size()); ++j) {
        ...
        errors.push_back((double)expected_values_[j] -
                         n.output);  // expected - actual
      }
    }
    for (int j = 0; j < (int)layer.neurons.size(); ++j) {
      Neuron& n = layer.neurons[j];
...
void GraphNetwork::UpdateExpectedValues(
    std::vector<std::vector<double>>& letters) {
...
void GraphNetwork::Train(int epochs_count, std::string filename,
                         bool cross_validation, int number_of_groups_k,
                         int test_group_number) {
  ...
  double sum = 0, mse = 0, total_mse = 0, cross_validation_mse = 0,
  ...
  if (cross_validation) {
    InitCrossValidation(number_of_groups_k, test_group_number,
                        cross_validation_j_min, cross_validation_j_max,
                        epochs_count, precisions_for_letters_vector,
    ...
    for (int j = 0; j < (int)(training_letters_.size()); ++j) {
      ...
      if (cross_validation) {
        if (j >= cross_validation_j_min && j < cross_validation_j_max) {
          ...
          int coincidence =
              (training_letters_[iterator_][0] == index + 1) ? 1 : 0;
          correct_answers_sum += coincidence;
          precisions_for_letters_vector[index].push_back(coincidence);
          recalls_for_letters_vector[training_letters_[iterator_][0] - 1]
              .push_back(coincidence);
          UpdateExpectedValues(training_letters_);
          PushBackMse(sum, mse, cross_validation_mse, mse_for_testing_,
                      j - cross_validation_j_min);
        } else {
          if (start_testing_time != 0) {
            time_spent_on_testing_ =
                (clock() - start_testing_time) / CLOCKS_PER_SEC;
          }
          BackProp();
          UpdateWeights();
        }  // ********************************************
      ...
      if ((j + 1) % 15000 == 0) {
        std::cout << "Обучено " << j + 1 << " букв " << i + 1 << "-й эпохи..."
                  << std::endl;
        std::cout << "Текущее значение MSE: " << total_mse / (j + 1)
                  << std::endl;
      }
      PushBackMse(sum, mse, total_mse, mse_for_training_, j);
      ++iterator_;
    }
    std::cout << i + 1 << "-я эпоха закончена. Среднее значение MSE: "
              << total_mse / training_letters_.size() << std::endl;
  }
  if (cross_validation) {
    CalculateTestingResults(
        correct_answers_sum, (training_letters_.size() / number_of_groups_k),
        precisions_for_letters_vector, recalls_for_letters_vector);
  }
  is_trained_ = true;
  time_spent_on_training_ =
      (clock() - start_training_time) / CLOCKS_PER_SEC - time_spent_on_testing_;
}

void GraphNetwork::InitCrossValidation(
    const int number_of_groups_k, const int test_group_number,
    int& cross_validation_j_min, int& cross_validation_j_max, int& epochs_count,
    std::vector<std::vector<int>>& precisions_for_letters_vector,
    std::vector<std::vector<int>>& recalls_for_letters_vector) {
  cross_validation_j_min =
      test_group_number == 1 ? 0
                             : (training_letters_.size() / number_of_groups_k) *
                                       (test_group_number - 1) -
                                   1;
  cross_validation_j_max =
      test_group_number == number_of_groups_k
          ? training_letters_.size() - 1
          : training_letters_.size() / number_of_groups_k * test_group_number -
                1;
  epochs_count = 1;
...
void GraphNetwork::PushBackMse(double& sum, double& mse, double& total_mse,
                               std::vector<double>& mse_vector, const int j) {
  sum = 0;
  for (int output_cnt = 0; output_cnt < output_neurons_count_; ++output_cnt) {
    double tmp = expected_values_[output_cnt] -
                 layers_.back().neurons[output_cnt].output;
    sum += tmp * tmp;
  }
...
void GraphNetwork::Test(double part_of_test_data, std::string filename) {
  ...
  for (int j = 0; j < testing_letters_.size() * part_of_test_data; ++j) {
    ...
    correct_answers_sum += coincidence;
    precisions_for_letters_vector[index].push_back(coincidence);
    recalls_for_letters_vector[testing_letters_[iterator_][0] - 1].push_back(
        coincidence);
    for (int output_cnt = 0; output_cnt < output_neurons_count_; ++output_cnt) {
      expected_values_[output_cnt] =
          testing_letters_[iterator_][0] == output_cnt + 1 ? 1 : 0;
    }
  ...
  CalculateTestingResults(
      correct_answers_sum, (testing_letters_.size() * part_of_test_data),
      precisions_for_letters_vector, recalls_for_letters_vector);
  time_spent_on_testing_ = (clock() - start_time) / CLOCKS_PER_SEC;
  std::cout << "time spent: " << time_spent_on_testing_ << " sec" << std::endl;
}

void GraphNetwork::CalculateTestingResults(
    const int correct_answers_sum, const double letters_size,
    std::vector<std::vector<int>>& precisions_for_letters_vector,
    std::vector<std::vector<int>>& recalls_for_letters_vector) {
  ...
  for (int i = 0; i < output_neurons_count_; ++i) {
    ...
    std::cout << "precision for letter " << (char)(i + 97) << ": "
              << precisions_[i] << std::endl;
  }
  for (int i = 0; i < output_neurons_count_; ++i) {
    recalls_.resize(output_neurons_count_);
    recalls_[i] = CalculateArithmeticMean(recalls_for_letters_vector[i]);
    std::cout << "recall for letter " << (char)(i + 97) << ": " << recalls_[i]
              << std::endl;
  }
  for (int i = 0; i < output_neurons_count_; ++i) {
    f_measures_.resize(output_neurons_count_);
    f_measures_[i] =
        2 * (precisions_[i] * recalls_[i] / (precisions_[i] + recalls_[i]));
    std::cout << "f measure for letter " << (char)(i + 97) << ": "
              << f_measures_[i] << std::endl;
  }
...
double GraphNetwork::CalculateArithmeticMean(
...
const double& GraphNetwork::GetTimeSpentOnTesting() {
  return time_spent_on_testing_;
}
const double& GraphNetwork::GetTimeSpentOnTraining() {
  return time_spent_on_training_;
}
const std::vector<double>& GraphNetwork::GetMseForTraining() {
  return mse_for_training_;
}
const std::vector<double>& GraphNetwork::GetMseForTesting() {
  return mse_for_testing_;
}
