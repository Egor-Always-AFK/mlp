...
namespace s21 {
void MatrixNetwork::InitNetwork(const std::vector<int>& config,
...
void MatrixNetwork::ClearVectors(bool precisions, bool recalls, bool f_measures,
...
void MatrixNetwork::ForwardProp(
    const std::vector<std::vector<double>>& letters) {
  ...
  for (int i = 0; i < size[layers_count_ - 1]; i++) {
    if (i != int(expect))
      neurons_err[layers_count_ - 1][i] =
          -neurons_val[layers_count_ - 1][i] *
          actFunc.UseDer(neurons_val[layers_count_ - 1][i]);
    else
      neurons_err[layers_count_ - 1][i] =
          (1.0 - neurons_val[layers_count_ - 1][i]) *
          actFunc.UseDer(neurons_val[layers_count_ - 1][i]);
  }
  ...
  for (int i = 0; i < layers_count_ - 1; ++i) {
    for (int j = 0; j < size[i + 1]; ++j) {
      for (int k = 0; k < size[i]; ++k) {
        weights[i](j, k) +=
            neurons_val[i][k] * neurons_err[i + 1][j] * learning_rate_;
      }
    }
  }
  for (int i = 0; i < layers_count_ - 1; i++) {
...
void MatrixNetwork::Train(int epochs_count, std::string filename,
  ...
  double sum = 0.0, mse = 0.0, total_mse = 0.0, cross_validation_mse = 0.0,
      ...
      if (cross_validation) {
        if (j >= cross_validation_j_min && j < cross_validation_j_max) {
          ...
          recalls_for_letters_vector[training_letters_[iterator_][0]].push_back(
              (training_letters_[iterator_][0] == idx) ? 1 : 0);
          PushBackMse(training_letters_, sum, mse, cross_validation_mse,
                      mse_for_testing_, j - cross_validation_j_min);
        } else {
  ...
  is_trained_ = true;
  time_spent_on_training_ = (clock() - start_training_time) / CLOCKS_PER_SEC -
                            (cross_validation ? time_spent_on_testing_ : 0);
  std::cout << "TIME: " << time_spent_on_training_ << " sec" << std::endl;
}

void MatrixNetwork::InitCrossValidation(
...
void MatrixNetwork::PushBackMse(std::vector<std::vector<double>>& letters,
                                double& sum, double& mse, double& total_mse,
  ...
  for (int output_cnt = 0; output_cnt < output_neurons_count_; ++output_cnt) {
    double tmp = expected_values_[output_cnt] -
                 neurons_val[layers_count_ - 1][output_cnt];
    sum += tmp * tmp;
  }
...
void MatrixNetwork::UpdateExpectedValues(
...
void MatrixNetwork::CalculateTestingResults(
...
double MatrixNetwork::CalculateArithmeticMean(
...
const std::vector<double>& MatrixNetwork::GetPrecisions() {
  return precisions_;
}
...
const double& MatrixNetwork::GetTimeSpentOnTesting() {
  return time_spent_on_testing_;
}
const double& MatrixNetwork::GetTimeSpentOnTraining() {
  return time_spent_on_training_;
}
const std::vector<double>& MatrixNetwork::GetMseForTraining() {
  return mse_for_training_;
}
const std::vector<double>& MatrixNetwork::GetMseForTesting() {
