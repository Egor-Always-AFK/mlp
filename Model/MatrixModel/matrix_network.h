...
class MatrixNetwork : public Network {
 public:
  ...
  void Train(int epochs, std::string filename, bool cross_validation,
             int number_of_groups_k, int test_gropu_number);
 ...
 private:
  ...
  void ClearVectors(bool precisions, bool recalls, bool f_measures,
  ...
  void PushBackMse(std::vector<std::vector<double>>& letters, double& sum,
                   double& mse, double& total_mse,
                   std::vector<double>& mse_vector, const int j);
  void UpdateExpectedValues(std::vector<std::vector<double>>& letters);
  void CalculateTestingResults(
